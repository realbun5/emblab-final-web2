...
export function getLightLevelScore(lightLevel) {
  //TODO: return 3 if time is night
...
function getTotalScore(data, isLoading) {
  ...
  const score =
    ...
    getLightLevelScore(data[data.length - 1].lightLevel);
  if (score >= 9) return 3;
...
export default function OverAllStatus() {
  ...
  return (
    <div className="w-5/12 h-auto flex flex-col flex-wrap bg-[#C6C6C6]">
      <h2
        className={`${
          isLoading ? "" : color_score[getTempScore(data[data.length - 1].temp)]
        } 
